...
//milestone III : segments according id
//map<unsigned, StreetSegmentInfo> segmentinfo;
...
bool load_map(std::string map_name) {
    ...
    for (unsigned i = 0; i < getNumberOfStreetSegments(); i++) {
       ...
       // segmentinfo.insert(make_pair(i,getStreetSegmentInfo(i)));
    }
    //Initialization
    float lat = 0;
...
double find_distance_between_two_points(LatLon point1, LatLon point2) {
    double c=cos((point1.lat + point2.lat) / 2 * DEG_TO_RAD);
    double X1 = point1.lon * c * DEG_TO_RAD; //get the position of point1 in XY coodinate
    double Y1 = point1.lat* DEG_TO_RAD;
    double X2 = point2.lon * c * DEG_TO_RAD; //get the position of point2 in XY coodinate
    double Y2 = point2.lat* DEG_TO_RAD;
    double d = EARTH_RADIUS_IN_METERS * sqrt(pow(Y1 - Y2, 2) + pow(X1 - X2, 2)); //get the distance
...
double find_street_segment_travel_time(unsigned street_segment_id) {
    ...
    double speed_limit = getStreetSegmentInfo(street_segment_id).speedLimit; // get the speed limit
...
double helper_find_distance(LatLon point1, LatLon point2) {
    ...
    double X1 = point1.lon * cos_lat_ave * DEG_TO_RAD; //get the position of point1 in XY coodinate
    double Y1 = point1.lat* DEG_TO_RAD;
    double X2 = point2.lon * cos_lat_ave * DEG_TO_RAD; //get the position of point2 in XY coodinate
    double Y2 = point2.lat* DEG_TO_RAD;
    double d = EARTH_RADIUS_IN_METERS * sqrt(pow(Y1 - Y2, 2) + pow(X1 - X2, 2)); //get the distance
...
unsigned find_closest_intersection(LatLon my_position) {
    ...
    double distance = helper_find_distance(my_position, getIntersectionPosition(0));
    unsigned storeid = 0;
    ...
    for (unsigned a = 1; a < getNumberOfIntersections(); a++) {
        ...
        double compare = helper_find_distance(my_position, getIntersectionPosition(a));
