...
vector<vector<unsigned>> samedrop;
...
vector<unsigned> traveling_courier(const std::vector<DeliveryInfo>& deliveries, const std::vector<unsigned>& depots) {
    ...
    for (unsigned count = 0; count < deliveries.size(); count++) {
        ...
        for (unsigned c2 = 0; c2 < deliveries.size(); c2++) {
            if (c2 == count) {
                continue;
            }
            if (deliveries[count].dropOff == deliveries[c2].dropOff) {
                ji.push_back(deliveries[c2].pickUp);
            }
        }

        samedrop.push_back(ji);
    ...
    for (unsigned o = 0; o < depots.size(); o++) {
        ...
        while (!todeliver.empty()) {
            ...
            for (unsigned a = 0; a < deliveries.size(); a++) {
                vector<unsigned> shu = samedrop[a];
                ...
                if (deliveries[a].pickUp == next) {
                    ...
                    for (unsigned b = 0; b < todeliver.size(); b++) {
                        ...
                        if (todeliver[b] == next) {
                            ...
                            if (shu.size() != 0) {
                                for (unsigned x = 0; x < shu.size(); x++) {

                                    if (find(todeliver.begin(), todeliver.end(), shu[x]) != todeliver.end()) {
                                        found = 1;
                                    }
                                }
                            }
